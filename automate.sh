...
PATH_OPENCV_BUILD=/root/opencv/build/
...
(cd build && cmake -DCMAKE_PREFIX_PATH=${PATH_LIBTORCH_DIR} -DCMAKE_MODULE_PATH=${PATH_LIBTORCH_BUILD} -DOpenCV_DIR=${PATH_OPENCV_BUILD} ..) || (echo "${bold}${red}cmake command failed... exiting.." && exit 0)
